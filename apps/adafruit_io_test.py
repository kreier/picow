...
# get feed
    ...
    picowTime_feed = io.get_feed("pitime")
except AdafruitIO_RequestError:
# if no feed exists, create one
    picowTemp_feed = io.create_new_feed("pitemp")
    picowTime_feed = io.create_new_feed("pitime")

#  pack feed names into an array for the loop
feed_names = [picowTemp_feed, picowTime_feed]
print("feeds created")
...
while True:
    try:
        ...
        if clock > 300:
            ...
            data = [cpu.temperature, time.time() - starttime]
            ...
            print("\nTemperature: %0.1f C" % cpu.temperature)
            print(f"Time since start: {time.time() - starttime} seconds")
            print()
            time.sleep(1)
            #  reset clock
            clock = 0
